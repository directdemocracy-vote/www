...
The vote includes a ballot number which is a random bits generated by the app and guarantying the uniqueness of the vote.
It also inclues the answer of the citizen to the referendum question.
It is encrypted using a temporary key generated by the app in a way it could be blind signed and
The registration publication is signed by the citizen.
...
Note that this publication is different from others because the signature refers only to the number and vote fields.
The published date of the vote publication is set to the deadline of the referendum.
