...
window.onload = function() {
    ...
    document.getElementById('endorse-button').addEventListener('click', function() {
          ...
          xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
              ...
              if (endorsed.hasOwnProperty('error') {
                ...
                document.getElementById('endorse-picture').src = endorsed.picture; document.getElementById('endorse-family-name').innerHTML = endorsed.familyName; document.getElementById('endorse-given-names').innerHTML = endorsed.givenNames; document.getElementById('endorse-coords').innerHTML = endorsed.latitude + ', ' + endorsed.longitude;
                //marker.setPopupContent(a.address.Match_addr + '<br><br><center style="color:#999">(' + lat + ', ' + lon + ')</center>').openPopup();
                //document.getElementById('register-address').innerHTML = a.address.Match_addr;
              }
            };
      ...
      clearForms(); $('.nav-tabs').on('shown.bs.tab', function(event) {
        if (map) map.invalidateSize();
      }); publisher = localStorage.getItem('publisher');
      if (!publisher) publisher = 'https://publisher.directdemocracy.vote'; document.getElementById('publisher').value = publisher; privateKey = localStorage.getItem('privateKey');
